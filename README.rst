...
Note, that although ``.. container:: prismjs-sphinx`` technically isn't
strictly required, it wraps our link into an element with ``prismjs-sphinx``
class and that's what allows us to safely hook to all underlying download
links without a risk to cause unwanted behavior for other places where you
might have used ``:download:`` directive for other purposes.
